...
void compute_gate( my_fixed fft_in_buf_real[BLOCK_NUM_QXR][FFT_SIZE],        // without ifft
                   my_fixed W_real[W_PXR_SIZE][W_QXR_SIZE],
                   my_fixed W_imag[W_PXR_SIZE][W_QXR_SIZE],
                   my_fixed ifft_out_buf_real[BLOCK_NUM_PXR][FFT_SIZE]) {
...
#pragma HLS ARRAY_PARTITION variable=fft_out_buf_imag complete dim=0
    ...
    my_fixed sum_real_p[FIXED_ADD_LAT][BLOCK_NUM_PXR][FFT_SIZE-3];
    my_fixed sum_imag_p[FIXED_ADD_LAT][BLOCK_NUM_PXR][FFT_SIZE-3];
...
loop_lstm_i: for (unsigned i = 0; i < BLOCK_NUM_PXR; i+=PLP_XR) {
    loop_lstm_j: for (unsigned j = 0; j < BLOCK_NUM_QXR; j+=FIXED_ADD_LAT) {
#pragma HLS PIPELINE II=2
    ...
    // perform sum final
    loop_sum_p: for (unsigned i = 0; i < BLOCK_NUM_PXR; ++i) {
#pragma HLS PIPELINE II=1
