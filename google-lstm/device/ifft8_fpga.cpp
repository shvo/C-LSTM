...
void ifft8_fixed(my_fixed x0_real[FFT_SIZE], my_fixed x0_imag[FFT_SIZE], my_fixed y0_real[FFT_SIZE]) {
#pragma HLS INLINE
#pragma HLS ARRAY_PARTITION variable=x0_real complete dim=1
#pragma HLS ARRAY_PARTITION variable=x0_imag complete dim=1
...
#pragma HLS ARRAY_PARTITION variable=x3_imag complete dim=1
    ...
    loop_stage_1: for (unsigned m = 0; m < 4; ++m) {
        ...
        x1_imag[m]   = x0_imag[m] + x0_imag[m+4];

        my_fixed d1_real = x0_real[m] - x0_real[m+4];
        my_fixed d1_imag = x0_imag[m] - x0_imag[m+4];
        x1_real[m+4] = d1_real * t1_real[m] - d1_imag * t1_imag[m];
        x1_imag[m+4] = d1_real * t1_imag[m] + d1_imag * t1_real[m];
    }
